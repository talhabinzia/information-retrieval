...
class Stemmer
{
	 ...
	 * file-name ...
	 */
	public static void stemFile(String[] filepath)
	{
		char[] w = new char[501];
		...
		try
		{
			File temfile = new File(filepath[0]);
			
			File statText = new File("final corpus/" +temfile.getName());
			...
			for (int i = 0; i < filepath.length; i++)
				try
				{
					...
					FileInputStream in = new FileInputStream(filepath[i]);
					...
					{
						System.out.println("error reading " + filepath[i]);
						break;
					}
				} catch (FileNotFoundException e)
				{
					System.out.println("file " + filepath[i] + " not found");
					break;
				}
			wr.close();
		} catch (IOException e)
